...
To run the tests for the certificate client you should activate the `cert_tests` feature, and use release mode in order to avoid lengthy key encryption/decryption times.
...
### Options
    ...
    --totp-token-name <TOTP_TOKEN_NAME>         Name of the TOTP token to be used for authentication (optional, only for exemption leaf certs).
...
### Arguments
    ...
    <TOKEN TYPE>                Type of token [possible values: keyserver, database, verifier, hlt, synthesizer]
...
### Synthesizer token options
    ...
    --audit-public-key-file <AUDIT_PUBLIC_KEY_FILE>
        File containing the public key of the audit recipient (optional).

The database, verifier, and HLT tokens have no custom options.

### Examples

    sdna-create-token keyserver --id 1
    sdna-create-token database
    sdna-create-token verifier
    sdna-create-token hlt
    sdna-create-token synthesizer --domain X.Y.Z --model X10L --serial pO4567 --rate-limit 100000 --audit-email a@example.com --audit-public-key-file ~/SecureDNA/audit.pub
...
A tool to facilitate encryption and decryption of screening results for the purpose of secure auditing.

- Generate new key pairs for encryption and decryption
- Encrypt data using a recipient's public key
...
### Commands
    ...
    create-key [--output <PRIVATE KEY FILE PATH>]
    encrypt --input <PLAINTEXT FILE PATH> --public-key <RECIPIENT PUBLIC KEY FILE PATH> [--output <ENCRYPTED FILE PATH>]
    decrypt --input <ENCRYPTED FILE PATH> --private-key <PRIVATE KEY FILE PATH> [--output <DECRYPTED FILE PATH>]
