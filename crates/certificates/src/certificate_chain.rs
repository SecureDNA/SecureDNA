...
impl<R: Role> CertificateChain<R> {
    pub fn new() -> Self {
        Self(BTreeSet::new())
    }

    pub fn add_item(&mut self, cert: impl Into<Certificate<R, KeyUnavailable>>) {
        self.0.insert(cert.into());
    }

    pub fn from_items<I, T>(certs: I) -> Self
    where
    ...
    pub fn add_items<I, T>(&mut self, certs: I)
    where
        I: IntoIterator<Item = T>,
        T: Into<Certificate<R, KeyUnavailable>>,
    {
...
impl<R: Role> IntoIterator for CertificateChain<R> {
    type Item = Certificate<R, KeyUnavailable>;
...
mod tests {
        ...
        RequestBuilder<R>: Builder<Item = CertificateRequest<R, KeyUnavailable>>,
    {
        ...
        CertificateChain::from_items(vec![root_cert.into_key_unavailable(), int_cert])
    }
}
