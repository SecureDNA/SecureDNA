...
mod tests {
    ...
    use certificates::{
        DatabaseTokenGroup, KeyserverTokenGroup, SynthesizerTokenGroup, VerifierTokenGroup,
    };
    ...
    #[test]
    fn check_test_verifier_token_bundle() {
        let base_path = PathBuf::from(TEST_DIR).join("verifier-token");
        let token_file: PathBuf = base_path.with_extension("vt");
        let key_file: PathBuf = base_path.with_extension("priv");
        let passphrase_file: PathBuf = base_path.with_extension("passphrase");

        let result = check_token_bundle_and_associated_key::<VerifierTokenGroup>(
            &token_file,
            &key_file,
            &passphrase_file,
            &TEST_INFRASTRUCTURE_ROOT,
        ...
        assert!(
            result.is_ok(),
            "Test verifier token checks failed: {:?}",
            result.err()
        );
    }
