...
use certificates::{CertificateBundle, ChainTraversal, PublicKey, Role, SystemClock};
...
macro_rules! validate_public_and_private_cert_files {
    ($test_name:ident, $certs_dir:expr, $root_public_key:expr, $role:ident; $($cert_type:expr),+) => {
        #[test]
        fn $test_name() {
            for ct in [$($cert_type,)+] {
                let base_path = ::std::path::PathBuf::from($certs_dir).join(format!(
                    "{}-{}",
                    stringify!($role).to_lowercase(),
                    ct
                ));
...
macro_rules! validate_public_cert_files {
    ($test_name:ident, $certs_dir:expr, $root_public_key:expr, $role:ident; $($cert_type:expr),+) => {
        ...
        fn $test_name() {
            for ct in [$($cert_type,)+] {
                let base_path = PathBuf::from($certs_dir).join(format!(
                    "{}-{}",
                    stringify!($role).to_lowercase(),
                    ct
                ));
